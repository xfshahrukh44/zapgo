...
class OrderController extends Controller
{
	...
	public function placeOrder(Request $request)
	{
		...
		$order->delivery_address_1 = $request->delivery_address_1;
		$order->delivery_address_2 = $request->delivery_address_2;
		$order->delivery_city = $request->delivery_city;
		$order->delivery_state = $request->delivery_state;
		$order->delivery_zip_code = $request->delivery_zip_code;
        ...
        $order->round_trip = $request->round_trip;
		...
		if ($order) {
			...
			foreach ($cart as $key => $value) {
				if ($value['name'] != '') {
                    ...
                    // $date_start = Carbon::createFromFormat('m-d-Y', $date_array[0]);
                    // $date_from = Carbon::createFromFormat('m-d-Y', $date_array[1]);
					...
					$order_products->order_products_name = $value['name'];
					$order_products->order_products_price = $value['price'];
					$order_products->orders_id = $orders->id;
					$order_products->order_products_qty = $value['qty'];
					$order_products->mat_language = $value['mat_language'];
            ...
            Session::flash('message', 'Please Sign In First');
