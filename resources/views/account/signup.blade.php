                        ...
                        <form method="POST" action="{{ route('register') }}">
                            ...
                            <div class="mb-3">
                                ...
                                <p>Your password must contain at least one uppercase letter (A-Z), one lowercase letter
                                    (a-z), one number (0-9), be at least eight characters long and can’t contain your first
                                    name, last name, or email.</p>
                            </div>

                            <div class="mb-3">
                                <label for="company_name" class="form-label">Company Name* why do we need your company name?
                                    ZapGO Rentals only. equipment rental to other Business.</label>
                                    ...
                                    <select class="form-select" id="user_state" name="state" required>
                                        ...
                                        @foreach ($state as $value)
                                            <option value="{{ $value->name }}"
                                                {{ old('state') == $value->name ? 'selected' : '' }}>{{ $value->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="age" id="yes"
                                        value="Yes" {{ old('age') == 'Yes' ? 'checked' : '' }} required>
                                    <label class="form-check-label" for="yes">Yes</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="age" id="no"
                                        value="No" {{ old('age') == 'No' ? 'checked' : '' }} required>
                            ...
                            <div class="mb-3">
                                ...
                                <p>Why do you need my ID information? For safety and security, you must be 18 years or older
                                    and have a valid state issued ID or Driver’s License to accept ZapGO Rentals equipment.
                                </p>
                                ...
                                <select class="form-select" id="choose_id" name="license_state" required>
                                    ...
                                    @foreach ($state as $value)
                                        <option value="{{ $value->name }}"
                                            {{ old('license_state') == $value->name ? 'selected' : '' }}>
                                            {{ $value->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="state_id" name="license_no"
                                ...
                                <select class="form-select" id="role" name="role" required>
                                    ...
                                    @foreach ($roles as $value)
                                        <option value="{{ $value->id }}"
                                            {{ old('role') == $value->id ? 'selected' : '' }}>{{ ucfirst($value->name) }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
